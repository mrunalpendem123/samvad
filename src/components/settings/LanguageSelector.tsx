  ...
  const filteredLanguages = useMemo(
    () =>
      LANGUAGES.filter((language) => {
        const matchesSearch = language.label.toLowerCase().includes(searchQuery.toLowerCase());

        if (!supportedLanguages || supportedLanguages.length === 0) {
          return matchesSearch;
        }

        // Backend sends full names like "Hindi", "English". 
        // LANGUAGES has labels like "Hindi", "English".
        // Attempt to match label.
        const isSupported = supportedLanguages.some(supported =>
          supported.toLowerCase() === language.label.toLowerCase()
        );
    ...
    >
      <div className="flex items-center space-x-1">
        <div className="relative" ref={dropdownRef}>
          <button
            type="button"
            className={`px-2 py-1 text-sm font-semibold bg-mid-gray/10 border border-mid-gray/80 rounded min-w-[200px] text-left flex items-center justify-between transition-all duration-150 ${isUpdating("selected_language")
                ...
                {filteredLanguages.length === 0 ? (
                  ...
                  filteredLanguages.map((language) => (
                    <button
                      key={language.value}
                      type="button"
                      className={`w-full px-2 py-1 text-sm text-left hover:bg-logo-primary/10 transition-colors duration-150 ${selectedLanguage === language.value
