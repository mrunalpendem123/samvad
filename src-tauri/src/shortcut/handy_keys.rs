...
impl HandyKeysState {
    ...
    pub fn new(app: AppHandle) -> Result<Self, String> {
        ...
        let thread_handle = thread::spawn(move || {
            Self::manager_thread(cmd_rx, app_clone, init_tx);
        });

        // Wait for manager initialization result
        match init_rx.recv() {
            Ok(Ok(())) => {}
            Ok(Err(e)) => {
            ...
            Err(_) => {
                let _ = thread_handle.join();
                return Err("Failed to initialize handy-keys manager".into());
            }
        }
        ...
        let manager = match HotkeyManager::new() {
            ...
            Err(e) => {
                let _ = init_tx.send(Err(format!(
                    "Failed to create HotkeyManager: {}",
                    e
                )));
        ...
        while running.load(Ordering::SeqCst) {
            ...
            if let Some(key_event) = event {
                ...
                let hotkey_string = key_event
                    ...
                    .unwrap_or_else(|_| build_hotkey_string(&modifiers, key.as_deref()));
...
fn build_hotkey_string(modifiers: &[String], key: Option<&str>) -> String {
