...
impl ModelManager {
    pub fn new(app_handle: &AppHandle) -> Result<Self> {
        ...
        available_models.insert(
            "nvidia/nemotron-speech-streaming-en-0.6b".to_string(),
            ModelInfo {
                id: "nvidia/nemotron-speech-streaming-en-0.6b".to_string(),
                name: "NVIDIA Nemotron 0.6B".to_string(),
                description: "NVIDIA Nemotron Speech Streaming 0.6B Model".to_string(),
                filename: "parakeet-tdt-0.6b-v3-int8".to_string(), // Keeping filename as is
        ...
        available_models.insert(
            "sarvam-speech-to-text".to_string(),
            ModelInfo {
                id: "sarvam-speech-to-text".to_string(),
                name: "Sarvam AI (Indic Languages)".to_string(),
                description: "Cloud-based speech-to-text for Indian languages via Sarvam AI".to_string(),
                filename: "sarvam_model_placeholder".to_string(),
                url: Some("https://api.sarvam.ai/speech-to-text".to_string()),
                size_mb: 0,
                is_downloaded: true,
                is_downloading: false,
                partial_size: 0,
                is_directory: false,
                engine_type: EngineType::Sarvam,
                accuracy_score: 0.95,
                speed_score: 0.95,
                languages: vec![
                    "Hindi".to_string(), "Tamil".to_string(), "Telugu".to_string(), 
                    "Malayalam".to_string(), "Kannada".to_string(), "Marathi".to_string(), 
                    "Gujarati".to_string(), "Bengali".to_string(), "Odia".to_string(), 
                    "Punjabi".to_string(), "English".to_string()
                ],
            },
        );
    ...
    fn migrate_bundled_models(&self) -> Result<()> {
        ...
        let bundled_models: [&str; 0] = []; // Removed "ggml-small.bin"
    ...
    fn update_download_status(&self) -> Result<()> {
        ...
        for model in models.values_mut() {
            ...
            if matches!(model.engine_type, EngineType::Sarvam) {
    ...
    fn auto_select_model_if_needed(&self) -> Result<()> {
        ...
        if settings.selected_model.is_empty() {
            let target_model_id = "nvidia/nemotron-speech-streaming-en-0.6b";
            info!("Auto-selecting default model: {}", target_model_id);

            // Update settings with the selected model
            let mut updated_settings = settings;
            updated_settings.selected_model = target_model_id.to_string();
            write_settings(&self.app_handle, updated_settings);

            info!("Successfully auto-selected model: {}", target_model_id);
        }
    ...
    pub async fn download_model(&self, model_id: &str) -> Result<()> {
        ...
        if matches!(model_info.engine_type, EngineType::Sarvam) {
            info!("Sarvam model is cloud-based, skipping download");
            return Ok(());
        }
    ...
    pub fn delete_model(&self, model_id: &str) -> Result<()> {
        ...
        if matches!(model_info.engine_type, EngineType::Sarvam) {
             info!("Sarvam model is cloud-based, nothing to delete");
             return Ok(());
        }
